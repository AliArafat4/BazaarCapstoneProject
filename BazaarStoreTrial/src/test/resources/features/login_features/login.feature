...
Feature: Login Functionality
  ...
  @Smoke @Customer
  Scenario: US02_TC001 - Successful login with valid customer credentials from config
    When user enters valid customer credentials
    And user clicks login button
    Then user should be logged in successfully

  @Smoke
  Scenario: US02_TC002 - Successful login with valid credentials
    When user enters email "customer@sda.com" and password "Password.12345"
    And user clicks login button
    Then user should be logged in successfully

  @Negative
  Scenario: US02_TC003 - Login with invalid credentials
